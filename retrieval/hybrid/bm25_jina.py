...
def _merge_hybrid(
    ...
    jina_hits: List[Dict[str, Any]],
    alpha: float
) -> List[Dict[str, Any]]:
    bm25_norm = _minmax_normalize(bm25_hits)
    jina_norm = _minmax_normalize(jina_hits)

    merged: Dict[str, Dict[str, Any]] = {}

    for hit in jina_hits:
    ...
    for doc_id, hit in merged.items():
        ...
        j = jina_norm.get(doc_id, 0.0)
        hybrid_score = alpha * b + (1.0 - alpha) * j
...
def hybrid_topk_effective(
    ...
    jina_fetch_k: Optional[int] = None,
    ...
    if jina_fetch_k is None:
        jina_fetch_k = max(k, 150)

    ref_bm25, bm25_hits = bm25_topk_effective(meta, k=bm25_fetch_k)
    ref_jina, jina_hits = knn_topk_effective(meta, k=jina_fetch_k)

    reference_date = ref_jina or ref_bm25

    if not bm25_hits and not jina_hits:
        return reference_date, []

    if not bm25_hits:
        only_jina = copy.deepcopy(jina_hits[:k])
        jina_norm = _minmax_normalize(jina_hits)
        for hit in only_jina:
            hit["_score"] = jina_norm.get(_doc_key(hit), 0.0)
        return reference_date, only_jina
    ...
    merged_hits = _merge_hybrid(bm25_hits, jina_hits, alpha=alpha)
